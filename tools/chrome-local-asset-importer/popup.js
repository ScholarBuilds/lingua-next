...
// ---- 心跳 ----
// Lingua 的连接器页面要显示「已连接 / 未连接」，判据只能是扩展自己说过话。
// 面板开着就按固定间隔报一次；关掉就自然停，后端超时后如实转成未连接。
const CONNECTOR_ID = 'chrome';
const CONNECTOR_HEARTBEAT_MS = 45000;
let heartbeatTimer = null;

function connectorQuery(){
  const version = chrome.runtime?.getManifest?.().version || '';
  return `connector=${CONNECTOR_ID}&connector_version=${encodeURIComponent(version)}`;
}

async function sendHeartbeat(){
  // 网络不通是常态（没启服务、地址填错），静默失败即可，别弹状态干扰操作
  try { await fetch(`${apiBase()}/api/studio/connectors/status?${connectorQuery()}`, {cache: 'no-store'}); }
  catch {}
}
...
function startHeartbeat(){
  ...
  sendHeartbeat();
  heartbeatTimer = setInterval(sendHeartbeat, CONNECTOR_HEARTBEAT_MS);
}
...
function updateSettingsUi(){
  ...
  const model = provider?.chat_models?.find(item => item.id === els.model.value);
  els.settingsSummary.textContent = `${apiBase().replace(/^https?:\/\//, '')}${provider ? ` · ${provider.name || provider.id}` : ''}${model ? ` · ${model.label}` : ''}`;
}
...
function renderProviders(){
  ...
  els.model.innerHTML = models.map(m => `<option value="${escapeHtml(m.id)}">${escapeHtml(m.label)}</option>`).join('');
  els.model.value = models.some(m => m.id === wantedModel) ? wantedModel : (models[0]?.id || '');
...
function getSettingsPayload(){
  return {
    server: els.server.value || '127.0.0.1:5173',
...
async function loadSettings(){
  ...
  els.server.value = data.server || (data.port ? `127.0.0.1:${data.port}` : '127.0.0.1:5173');
...
async function loadProviders(){
  const [healthRes, settingsRes, deploymentsRes] = await Promise.all([
    fetch(`${apiBase()}/api/healthz`),
    fetch(`${apiBase()}/api/studio/assets/settings`),
    fetch(`${apiBase()}/api/config/model-deployments?media_type=chat&enabled=true`),
  ]);
  for(const res of [healthRes, settingsRes, deploymentsRes]){
    if(!res.ok) throw new Error(await res.text());
  }
  await healthRes.json();
  assetTagSettings = await settingsRes.json();
  const deployments = await deploymentsRes.json();
  const deploymentById = new Map((deployments || []).map(item => [String(item.id), item]));
  const globalDeployment = assetTagSettings?.deployment_id
    ? deploymentById.get(String(assetTagSettings.deployment_id))
    : null;
  const defaultLabel = globalDeployment
    ? `全局设置 · ${globalDeployment.display_name || globalDeployment.upstream_model_id}`
    : '默认视觉模型（explain-standard）';
  const grouped = new Map();
  (deployments || []).forEach(item => {
    const providerId = `credential-${item.credential_id}`;
    if(!grouped.has(providerId)){
      grouped.set(providerId, {
        id: providerId,
        name: item.credential_name || item.provider_type || `凭据 #${item.credential_id}`,
        enabled: true,
        chat_models: [],
      });
    }
    grouped.get(providerId).chat_models.push({
      id: String(item.id),
      label: item.display_name || item.upstream_model_id || `部署 #${item.id}`,
    });
  });
  // 扩展只持有部署 ID；真实凭据和 API Key 仍由 Lingua 服务端保管。
  providers = [{
    id: 'lingua-default',
    name: 'Lingua 默认绑定',
    enabled: true,
    chat_models: [{id: '', label: defaultLabel}],
  }, ...grouped.values()];
  renderProviders();
}
...
async function importSelected(){
  ...
  const body = {
    group_name: els.folder.value || '网页采集',
    auto_tag: Boolean(els.classify.checked),
    tag_deployment_id: els.classify.checked && els.model.value ? Number(els.model.value) : null,
    tag_prompt: els.classify.checked ? String(els.prompt.value || '').trim() : '',
  ...
  const res = await fetch(`${apiBase()}/api/studio/connectors/import?${connectorQuery()}`, {
    method: 'POST',
...
function safeFolderName(value){
  return String(value || '网页采集')
    .split(/[\\/]+/)
    .map(part => part.replace(/[<>:"|?*\x00-\x1F]/g, '_').replace(/^\.+$/, '_').trim())
    .filter(Boolean)
    .join('/') || '网页采集';
}
...
function downloadFileName(item, contentType){
  ...
  base = base.replace(/[<>:"|?*\\/\x00-\x1F]/g, '_').slice(0, 80) || 'web-media';
...
(async function init(){
  ...
  catch { setStatus('请输入服务地址后点击连接，例如 192.168.1.10:5173。'); }
})();
