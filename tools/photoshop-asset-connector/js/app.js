...
(function () {
  ...
  function startHeartbeat() {
    ...
    net.heartbeat();
    heartbeatTimer = setInterval(() => { if (state.connected) net.heartbeat(); }, HEARTBEAT_MS);
  }

  async function connect() {
    const host = net.parseHost(els.server.value);
    if (!host) { setConnMsg('请输入地址，例如 192.168.1.10:5173', 'err'); return; }
    state.host = host;
  ...
  els.github.addEventListener('click', () => {
    ps.openUrl(net.httpBase()).catch((err) => setConnMsg(`打开 Lingua Web 失败：${err.message || err}`, 'err'));
  });
  ...
  /* ---------- 初始化 ---------- */
  (function init() {
    els.server.value = localStorage.getItem(DX.LS.host) || '127.0.0.1:5173';
