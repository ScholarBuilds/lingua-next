...
import { enhanceHistoryItems, enhanceSourceAssetId, enhanceTaskAssetIds } from './enhance-history'

function task(patch: Partial<StudioTask> = {}): StudioTask {
  return {
    id: 'enhance-1', domain: 'studio', tool_id: 'enhance', task_type: 'workflow.comfyui',
    parent_task_id: null, batch_id: null, source_route: '/studio/enhance', source_context: null,
...
describe('local enhance history projection', () => {
  it('reads both workflow result shapes and source ids', () => {
    expect(enhanceTaskAssetIds(task({
      result: { asset_ids: [2, 2], items: [{ asset_id: '3' }] },
    }))).toEqual([2, 3])
    expect(enhanceSourceAssetId(task({ source_context: { source_asset_id: '7' } }))).toBe(7)
  })

  it('keeps final two-phase provenance and hides intermediate tasks', () => {
    const final = task({
      result: { items: [{ asset_id: 9 }] },
      source_context: {
        enhance_engine: 'local', source_asset_id: 1, strength: 0.72,
        upscale_resolution: 4096, history_visible: true,
      },
    })
    expect(enhanceHistoryItems(final, [asset(1), asset(9)]))
      .toEqual([expect.objectContaining({
        source: expect.objectContaining({ id: 1 }), strength: 0.72, resolution: 4096,
      })])
    expect(enhanceHistoryItems(task({
      result: { asset_ids: [8] },
      source_context: { enhance_engine: 'local', history_visible: false },
    }), [asset(8)])).toEqual([])
  })

  it('ignores generic model enhancement and archived outputs', () => {
    expect(enhanceHistoryItems(task({ result: { asset_ids: [4] } }), [asset(4)])).toEqual([])
    expect(enhanceHistoryItems(task({
      result: { asset_ids: [4] }, source_context: { enhance_engine: 'local' },
